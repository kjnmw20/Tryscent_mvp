    ...
    <div className="relative shrink-0 size-[24px]">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
        <circle
          ...
          strokeWidth="1.5"
        />
        {isSelected && <circle cx="12" cy="12" r="6" fill="black" />}
      </svg>
    </div>
  );
}
    ...
    >
      <div
        ...
        className={`absolute border-2 ${
          isSelected ? "border-black" : "border-[#e5e7eb]"
...
function FixedButton({ isEnabled, selectedOption }: { isEnabled: boolean, selectedOption: string | null }) {
  return (
    <div className="absolute bottom-0 left-0 right-0 bg-white z-10">
      <div
        aria-hidden="true"
        className="absolute border-[#f3f4f6] border-solid border-t inset-0 pointer-events-none"
      />
      <div className="pt-[25px] px-[24px] pb-[24px]">
        {isEnabled ? (
          <Link 
            ...
            onClick={() => {
              if (selectedOption) {
                const val = selectedOption === "여자" ? "FEMALE" : "MALE";
                sessionStorage.setItem("q1", val);
              }
            }}
          >
...
export default function SurveyGender() {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "linear-gradient(rgb(255, 255, 255) 0%, rgb(253, 253, 254) 33.333%, rgb(251, 252, 252) 66.667%, rgb(249, 250, 251) 100%), linear-gradient(90deg, rgb(255, 255, 255) 0%, rgb(255, 255, 255) 100%)",
      }}
    >
      <div className="bg-white h-screen w-[390px] max-w-full mx-auto shadow-[0px_20px_25px_0px_rgba(0,0,0,0.1),0px_8px_10px_0px_rgba(0,0,0,0.1)] relative">
        ...
        <FixedButton isEnabled={selectedOption !== null} selectedOption={selectedOption} />
      </div>
    </div>
  );
}
